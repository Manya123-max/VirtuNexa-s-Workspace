...
def fetch_news_from_sources(selected_sources, selected_category):
    ...
    for source in selected_sources:
        url = sources.get(source, source)  # Use predefined URL or user-entered custom URL
        all_headlines[source] = fetch_news(url, selected_category)
        store_history(source, selected_category)  # Store the history in the database
    return all_headlines


# Function to fetch news headlines based on category
def fetch_news(url, category):
    try:
        ...
        if category.lower() != "general":
            category_filtered = [headline for headline in filtered_headlines if category.lower() in headline.lower()]
            return category_filtered if category_filtered else ["No relevant headlines found for this category."]
...
def gui_app():
    ...
    close_button = tk.Button(root, text="Close", command=root.destroy, bg="red", fg="white", font=("Arial", 12, "bold"))
